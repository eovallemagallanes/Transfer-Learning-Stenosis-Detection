...
__suball__ = ['resnet18', 'resnet34', 'resnet50']
...
class MyResNet(nn.Module):
    ...
    def __init__(
        ...
        if self.num_blocks > len(layers) or self.num_blocks < 1:
            raise ValueError("num_blocks should be an integer betwwen 1 and {} ".format(len(layers)))
...
def resnet18(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> MyResNet:
...
def resnet34(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> MyResNet:
...
def resnet50(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> MyResNet:
...
def ResidualNet(model_name, num_blocks, num_classes, num_trainable_layers=-1, pretrained=True):
    if model_name not in __suball__:
        raise ValueError("model_name should be an string in set: {} ".format(__suball__))

    if model_name == 'resnet18':
        model = resnet18(num_blocks=num_blocks, num_classes=num_classes, pretrained=pretrained)
    if model_name == 'resnet34':
        model = resnet34(num_blocks=num_blocks, num_classes=num_classes, pretrained=pretrained)
    if model_name == 'resnet50':
        model = resnet50(num_blocks=num_blocks, num_classes=num_classes, pretrained=pretrained)
